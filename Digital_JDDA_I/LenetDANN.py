...
class Lenet(object):
    def __init__(self,inputs, scope='lenet', training_flag=True, reuse=False,l=1):
        self.scope=scope
        self.inputs=inputs
        ...
        with tf.variable_scope(self.scope, reuse=self.reuse):
            with slim.arg_scope([slim.fully_connected], activation_fn=tf.nn.relu):
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, padding='VALID'):
                    ...
                    feat=flip_gradient(net,self.l)
                    self.dann=slim.fully_connected(feat,100,activation_fn=tf.nn.relu, scope='d1')
                    self.adv=slim.fully_connected(self.dann,2,activation_fn=None,scope='d2')
