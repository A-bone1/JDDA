...
class BatchPreprocessor(object):
    ...
    def __init__(self, dataset_file_path, num_classes, output_size=[227, 227], horizontal_flip=False, shuffle=False,
        ...
        self.EdgeWeights = self.Label2EdgeWeights(self.labels)
    ...
    def shuffle_data(self):
        ...
        self.EdgeWeights = self.EdgeWeights[idx, :]
        self.EdgeWeights= self.EdgeWeights[:, idx]
        for i in idx:
    ...
    def next_batch(self, batch_size):
        ...
        weights = self.EdgeWeights[self.pointer: (self.pointer+batch_size), self.pointer: (self.pointer+batch_size)]
    ...
    def Label2EdgeWeights(self,Label):
        ...
        n = Label0.shape
        n = n[0]
        EdgeWeights = np.zeros((n, n))
        Label0 = np.expand_dims(Label0, axis=0)
        EdgeWeights = np.transpose(1.0 / Label0) * Label0
        indx, indy = np.where(EdgeWeights != 1.0)
